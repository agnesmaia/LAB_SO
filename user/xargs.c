...
#define MAX_ARG_LEN 512 // tamanho máximo permitido para cada argumento


void copy_argv(char **ori_argv, int ori_argc, char *new_argv, char **argv) {
// copia os argumentos da linha de comando e passa para um array "argv"
  int k = 0;
...
// imprime os argumentos de um comando  
  for (int i = 0; i < n; i++) {
    printf("%s\n", s[i]);
  }
}
...
// recebe os argumentos da linha de comando  
  ...
  char ch;
  int ignore = 0;
  // programa le a entrada e a cada linha executa o comando correspondente. O comando é executado em um processo filho.
  while (read(0, &ch, 1) > 0) {
    if (ch == '\n') {
      if (ignore) {
        i = 0;
